from deepref.encoder.bert_encoder import BERTEntityEncoder
...
def upos2id_and_deps2id():
    ...
    upos2id = json.loads(open(os.path.join('deepref', 'data', f'{DATASET}_upos2id.json'), 'r').read())
    deps2id = json.loads(open(os.path.join('deepref', 'data', f'{DATASET}_deps2id.json'), 'r').read())
