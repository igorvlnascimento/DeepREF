...
The first two are the datasets and metrics already explained above. The `-t` arg means the number of trials that Optuna needs to execute to find the best combination of parameters. The `-o` is the type of optimization that you can choose between `hyperparams` or `model`. If you choose `hyperparams` (default), Optuna will find the best hyperparameters for the model and embedding indicated on `deepref/data/best_hparams_<dataset>.json` file. The default model and embedding are `bert` and `bert-base-uncaseed`, respectively. If you choose the `model` option, Optuna will find the best model and embedding combination. The possible models and embeddings to be choosen can be seen on `deepref/config.py` file.
...
```
python deepref/optimization/preprocess_optimization.py -d <dataset> -m <metric>
```
...
```
python deepref/optimization/embedding_optimization.py -d <dataset> -m <metric>
```
...
```
python deepref/dataset/preprocess_dataset.py -d <dataset>
```

Dataset can be one of theses options: `semeval2010` (default), `semeval20181-1`, `semeval20181-2` and `ddi`.

This code will download the dataset, transform into a .csv file in a standard format and preprocess it to the preprocessing types specified on `deepref/data/best_hparams_<dataset>.json` file. The preprocessing types can be a combination of `eb` (entity blinding), `nb` (NER blinding), `d` (digit blinding), `b` (text between brackets or parenthesis removal), `p` (punctuation removal) and `sw` (stopwords removal). The preprocessing variable on `deepref/data/best_hparams_<dataset>.json` file is an integer. This integer represents the possible combinations of preprocessing. For example, preprocessing number 16 is ['b', 'd'] which means a combination of digit blindign and text between brackets or parenthesis removal. You can consult the list of preprocessing combinations on PREPROCESSING_COMBINATION variable in `deepref/config.py` file. The preprocessing number 0 means that the model will train on original dataset (without preprocessing).

For example, if you do this:
```
python deepref/dataset/preprocess_dataset.py -d semeval2010
```

It will download SemEval 2010 dataset, if it doesn't exists in the framework, transform to a .csv file in a standard format and preprocess dataset to the specified preprocessing type on `deepref/data/hyperparams_<dataset>.json` file. It gets the prepocessing value in this file which is an integer. This integer is the preprocessing combination index for the `PREPROCESSING_COMBINATION` list on `deepref/config.py` file.
...
```
 ...
 The `general` type loads a more general model for Stanza or SpaCy and the `scientific` loads the biomedical model for Stanza and the same general model for SpaCy. The file `deepref/data/nlp_config.py` has the NLP tool and model configuration. You can add new NLP models to this file. Don't forget to add to `NLP_MODELS` variable on `deepref/config.py` file.
