...
The POS and Deps encoding are optional and you can turn it off in `pos_embed` and `deps_embed` variables, respectively. This variables can be found in `opennre/data/best_hparams_<dataset>.json` file. 
...
This code will download the dataset, transform into a .csv file in a standard format and preprocess it to the preprocessing types specified on `opennre/data/best_hparams_<dataset>.json` file. The preprocessing types can be a combination of `eb` (entity blinding), `nb` (NER blinding), `d` (digit blinding), `b` (text between brackets or parenthesis removal), `p` (punctuation removal) and `sw` (stopwords removal). The preprocessing variable on `opennre/data/best_hparams_<dataset>.json` file is an integer. This integer represents the possible combinations of preprocessing. For example, preprocessing number 16 is ['b', 'd'] which means a combination of digit blindign and text between brackets or parenthesis removal. You can consult the list of preprocessing combinations on PREPROCESSING_COMBINATION variable in `opennre/config.py` file. The preprocessing number 0 means that the model will train on original dataset (without preprocessing).
