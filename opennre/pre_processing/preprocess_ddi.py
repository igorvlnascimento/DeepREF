...
def preprocess_ddi():
    ...
    converter = ConverterDDI()
    preprocess = Preprocess()

    df_train_drugbank = converter.get_dataset_dataframe(nlp, res('Train/DrugBank/'), relation_extraction=True)
    df_train_medline = converter.get_dataset_dataframe(nlp, res('Train/MedLine/'), relation_extraction=True)

    df_test_drugbank = converter.get_dataset_dataframe(nlp, res('Test/Test for DDI Extraction task/DrugBank/'), relation_extraction=True)
    df_test_medline = converter.get_dataset_dataframe(nlp, res('Test/Test for DDI Extraction task/MedLine/'), relation_extraction=True)
    ...
    converter.write_dataframe(df_train_drugbank, out(outdir + 'train_drugbank_original.csv'))
    df_train_drugbank_copy = converter.read_dataframe(out(outdir + 'train_drugbank_original.csv'))

    # The first checks with the pd.equals method, and the other does a manual checking per column
    converter.check_equality_of_written_and_read_df(df_train_drugbank, df_train_drugbank_copy)

    converter.write_dataframe(df_train_medline, out(outdir + 'train_medline_original.csv'))
    df_train_medline_copy = converter.read_dataframe(out(outdir + 'train_medline_original.csv'))
    converter.check_equality_of_written_and_read_df(df_train_medline, df_train_medline_copy)

    converter.write_dataframe(df_test_drugbank, out(outdir + 'test_drugbank_original.csv'))
    df_test_drugbank_copy = converter.read_dataframe(out(outdir + 'test_drugbank_original.csv'))
    converter.check_equality_of_written_and_read_df(df_test_drugbank, df_test_drugbank_copy)

    converter.write_dataframe(df_test_medline, out(outdir + 'test_medline_original.csv'))
    df_test_medline_copy = converter.read_dataframe(out(outdir + 'test_medline_original.csv'))
    converter.check_equality_of_written_and_read_df(df_test_medline, df_test_medline_copy)

    converter.write_into_txt(df_train_drugbank, out(outdir + 'train_drugbank_original.txt'))
    converter.write_into_txt(df_train_medline, out(outdir + 'train_medline_original.txt'))
    converter.write_into_txt(df_test_drugbank, out(outdir + 'test_drugbank_original.txt'))
    converter.write_into_txt(df_test_medline, out(outdir + 'test_medline_original.txt'))

    converter.combine(out, outdir,  'train_drugbank_original', 'train_medline_original', converter.dataset_name + '_train_original.txt')
    converter.combine(out, outdir, 'test_drugbank_original', 'test_medline_original', converter.dataset_name + '_val_original.txt')
    ...
    for original_df_name in original_dataframe_names:
        type1 = preprocess(converter.read_dataframe, out(indir + original_df_name + '_original.csv'), nlp)
    ...
    for i in ['train', 'test']:
        if i == 'train':
            converter.combine(out, outdir1, i + '_drugbank_entity_blinding', i + '_medline_entity_blinding', converter.dataset_name + '_train_entity_blinding.txt')
            converter.combine(out, outdir2, i + '_drugbank_punct_stop_digit', i + '_medline_punct_stop_digit', converter.dataset_name + '_train_punct_stop_digit.txt')
            converter.combine(out, outdir3, i + '_drugbank_punct_digit', i + '_medline_punct_digit', converter.dataset_name + '_train_punct_digit.txt')
            converter.combine(out, outdir4, i + '_drugbank_ner_blinding', i + '_medline_ner_blinding', converter.dataset_name + '_train_ner_blinding.txt')
        else:
            converter.combine(out, outdir1, i + '_drugbank_entity_blinding', i + '_medline_entity_blinding', converter.dataset_name + '_val_entity_blinding.txt')
            converter.combine(out, outdir2, i + '_drugbank_punct_stop_digit', i + '_medline_punct_stop_digit', converter.dataset_name + '_val_punct_stop_digit.txt')
            converter.combine(out, outdir3, i + '_drugbank_punct_digit', i + '_medline_punct_digit', converter.dataset_name + '_val_punct_digit.txt')
            converter.combine(out, outdir4, i + '_drugbank_ner_blinding', i + '_medline_ner_blinding', converter.dataset_name + '_val_ner_blinding.txt')
