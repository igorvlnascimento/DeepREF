...
from opennre.dataset.converters.converter_semeval2018 import get_dataset_dataframe, write_dataframe,\
...
def preprocess_semeval2018():
    ...
    df_train = get_dataset_dataframe(nlp, res('Train/'))
    df_test = get_dataset_dataframe(nlp, res('Test/'))

    if not os.path.exists(out(outdir)):
        os.makedirs(out(outdir))

    write_dataframe(df_train, out(outdir + 'semeval2018_train_original.csv'))

    df_train_copy = read_dataframe(out(outdir + 'semeval2018_train_original.csv'))
    ...
    write_dataframe(df_test, out(outdir + 'semeval2018_val_original.csv'))

    df_test_copy = read_dataframe(out(outdir + 'semeval2018_val_original.csv'))

    check_equality_of_written_and_read_df(df_test, df_test_copy)

    write_into_txt(df_train, out(outdir + 'semeval2018_train_original.txt'))

    write_into_txt(df_test, out(outdir + 'semeval2018_val_original.txt'))

    original_dataframe_names = ['semeval2018_train', 'semeval2018_val']
    makedir(outdir1, out)
    makedir(outdir2, out)
    makedir(outdir3, out)
    makedir(outdir4, out)
    ...
    print(output_file_length(out, indir + 'semeval2018_train_original.txt'))
    print(output_file_length(out, outdir1 + 'semeval2018_train_entity_blinding.txt'))
    print(output_file_length(out, outdir2 + 'semeval2018_train_punct_stop_digit.txt'))
    print(output_file_length(out, outdir3 + 'semeval2018_train_punct_digit.txt'))
    print(output_file_length(out, outdir4 + 'semeval2018_train_ner_blinding.txt'))

    print(output_file_length(out, indir + 'semeval2018_val_original.txt'))
    print(output_file_length(out, outdir1 + 'semeval2018_val_entity_blinding.txt'))
    print(output_file_length(out, outdir2 + 'semeval2018_val_punct_stop_digit.txt'))
    print(output_file_length(out, outdir3 + 'semeval2018_val_punct_digit.txt'))
    print(output_file_length(out, outdir4 + 'semeval2018_val_ner_blinding.txt'))
