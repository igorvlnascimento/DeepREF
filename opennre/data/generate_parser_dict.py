...
def csv2id(dataset_name):
    ...
    upos_list = list(train_df['upos_sentence']) + list(val_df['upos_sentence']) + list(test_df['upos_sentence'])
    upos_counter_dict = dict(Counter([item for sublist in [upos.split() for upos in upos_list] for item in sublist]))
    deps_list = list(train_df['deps_sentence']) + list(val_df['deps_sentence']) + list(test_df['deps_sentence'])
...
def save2json(dataset_name, upos_dict, deps_dict):
    with open(f'opennre/data/{dataset_name}_upos2id.json', 'w') as json_file:
        json_file.write(json.dumps(upos_dict))
    
    with open(f'opennre/data/{dataset_name}_deps2id.json', 'w') as json_file:
