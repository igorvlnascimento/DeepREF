...
class WordEmbeddingLoader(object):
    ...
    def load_embedding(self):
        ...
        if 'elmo' == self.embedding_name:
            ...
            with open(os.path.join(self.path_word, 'wiki-300d-1M.vec'), 'r', encoding='utf-8') as fr:
                for line in fr:
                    print(line)
                    line = line.strip().split()
                    if len(line) != self.word_dim + 1:
            ...
            fr = open(os.path.join(self.path_word, 'crawl-300d-2M.vec'), 'r', encoding='utf-8').readlines()
